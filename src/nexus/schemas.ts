...
const numericStringToIntSchema = numericStringSchema.transform(s => Number.parseInt(s));
const numberAndTransformIntStringSchema = z.number().or(numericStringToIntSchema);
const numberToStringSchema = z.number().transform(n => n.toString());
const stringAndTransformNumbersSchema = numericStringSchema.or(numberToStringSchema);

export const gameIdArgsSchema = z.object({ gameId: z.string() });
export const modIdArgsSchema = gameIdArgsSchema.merge(z.object({ modId: numberAndTransformIntStringSchema }));
export const fileIdArgsSchema = modIdArgsSchema.merge(z.object({ fileId: numberAndTransformIntStringSchema }));
export const endorseModArgsSchema = modIdArgsSchema.merge(z.object({ modVersion: z.string(), endorseStatus: z.literal('endorse').or(z.literal('abstain')) }));
export const getCollectionArgsSchema = z.object({ slug: z.string() });
export const getCollectionRevisionArgsSchema = z.object({ collectionSlug: z.string(), revisionNumber: z.number() });
export const getDownloadUrlArgsSchema = fileIdArgsSchema.merge(z.object({ key: z.string().optional(), expires: z.number().optional() }));
export const getFileByMd5ArgsSchema = gameIdArgsSchema.merge(z.object({ hash: z.string() }));
export const getMyCollectionsArgsSchema = gameIdArgsSchema.merge(z.object({ count: numberAndTransformIntStringSchema.optional(), offset: numberAndTransformIntStringSchema.optional() }));
export const getRecentlyUpdatedModsArgsSchema = gameIdArgsSchema.merge(z.object({ period: z.union([z.literal('1d'), z.literal('1w'), z.literal('1m')]) }));
export const rateCollectionRevisionArgsSchema = z.object({ revisionId: numberAndTransformIntStringSchema, rating: z.number() });
export const resolveCollectionUrlArgsSchema = z.object({ apiLink: z.string() });
export const trackModArgsSchema = gameIdArgsSchema.merge(z.object({ modId: stringAndTransformNumbersSchema }));
